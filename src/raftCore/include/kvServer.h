...
#include "rpcHander.h"
class KvServer : raftKVRpcProctoc::kvServerRpc::Service {
 private:
  std::mutex m_mtx;
  int m_me;
 ...
 public:
  ...
  void Get1(const raftKVRpcProctoc::GetArgs *args,
  ...
  // clerk 使用RPC远程调用
  void PutAppend1(const raftKVRpcProctoc::PutAppendArgs *args, raftKVRpcProctoc::PutAppendReply *reply);
