...
// 处理来自clerk的Get RPC
void KvServer::Get1(const raftKVRpcProctoc::GetArgs *args, raftKVRpcProctoc::GetReply *reply) {
  Op op;
  op.Operation = "Get";
  op.Key = args->key();
  op.Value = "";
...
void KvServer::PutAppend1(const raftKVRpcProctoc::PutAppendArgs *args, raftKVRpcProctoc::PutAppendReply *reply) {
  Op op;
...
grpc::Status KvServer::PutAppend(grpc::ServerContext* context, const ::raftKVRpcProctoc::PutAppendArgs* request, ::raftKVRpcProctoc::PutAppendReply* response) {
  KvServer::PutAppend1(request, response);
...
grpc::Status KvServer::Get(grpc::ServerContext* context, const ::raftKVRpcProctoc::GetArgs* request, ::raftKVRpcProctoc::GetReply* response) {
  KvServer::Get1(request, response);
  return grpc::Status::OK;
}
KvServer::KvServer(int me, int maxraftstate, std::string nodeInforFileName, short port) : m_skipList(6) {
  ...
  // });
  // t.detach();
  GrpcHander grpcHander;
  grpcHander.NotifyService(m_raftNode.get());
  grpcHander.Run(m_me, port);
  ...
  std::vector<std::shared_ptr<RaftRpcUtil> > servers; // 与别的raft node rpc连接的句柄
  //进行连接
